...
纯原生 Go 后端（chi + pgx，无 ORM），面向高并发（十万级在线 / 万级+ QPS）、强 AI、强扩展性；
业务建模贴合中国公路货运实务（运费承担方 / 到付回单付月结 / 代收货款 / 多计费方式 / 派车合规），
安全侧 RBAC 全端点强制 + 数据权限全域生效 + 登录审计，前端为产品级设计系统。

- `backend-go/` — Go 1.25 网关（chi + pgx 裸 SQL），单进程承载全部 API、内嵌 schema 迁移、
  进程内削峰与定时扫描；UUIDv7 主键，统一响应封装。迁移过程与契约差异见
  [PORTING.md](backend-go/PORTING.md)。
...
- **订单池与调度台**：确认进池 → **多调度并发认领**（行锁防抢单）→ **AI 派单建议**（系统运力池 + 多承运商比价 + 外部信号）→ 派单（自有单车/车队/三方承运商）转运单。
...
- **控制塔与实时**：KPI 态势、轨迹**进程内有界队列削峰 + 批量落库**、ETA/回单定时扫描预警（网关内周期协程）。
- **异常与回单**：异常闭环（分级/指派/处理/关闭 + 费用责任）、回单上传 + **可插拔 OCR**（未接真实引擎时**诚实降级为待人工核验，绝不伪造签收人/证件到期**，不覆盖人工录入）、**司机/客户签收回传（e-POD：电子签名 + 一步推进签收 → 触发订单完成）**。
- **订单溯源与通知**：订单全生命周期事件溯源（建单→完成全程留痕 + 时间线）、站内**通知中心**（进池通知调度 / 完成通知客服，未读角标轮询）。
- **费用与开放平台**：费用字典、报价规则、应收/应付自动归集、费用/付款接口、**Webhook 网关（HMAC + 重试）**。
- **对账单（ERP 财务）**：按客户(应收)/承运商(应付)在账期内归集费用，**对账单生成 → 明细快照 → 人工确认**，支持对方金额**差异稽核**（diff）。
- **AI 工作台**：手写 **ReAct 循环**（DeepSeek 作为 OpenAI 兼容 LLM）编排 9 个工具（ETA风险/回单/费用风控/调度建议/异常分析/客服话术/报警摘要/指标查询/拼单配载），对话状态落 Postgres；证据链 + **人工确认闭环**，AI 只建议不自动执行高风险动作。每个工具同时保留 REST 端点。
- **智能调度/排线**：可用运力匹配 + **装载适配评分**（紧凑装载优先）+ **多承运商比价**（价低者得）+ 批量**贪心排线**；调度只产出建议/计划，落地仍走人工派车。
- **调度指挥中心**：一屏指挥大屏——实时地图 + KPI 摘要（在线运力/待调度/在途/报警）+ 待调度池（逐单 **AI 调度建议**）+ **一键 AI 排线** + 报警流。
- **可扩展 Agent**：工具支持 **per-tool 参数 schema**（接入参数各异的大量 API 无需改代码）、**风险分级闸门**（低风险自动执行 / 高风险落建议待人工确认）、以及 **MCP 接入**（`AGENT_MCP_SERVERS` 配置外部 MCP server，网关自实现 Streamable HTTP 上的 JSON-RPC 客户端，拉到的工具与内置工具同表注册；标 `risk:"high"` 的 server 其工具不进自动循环，只登记待人工确认）。
- **车联网监控（GPS/IoT）**：车载终端建模（GPS/北斗/温度/油耗/ETC/ADAS/DSM），设备上报**进程内队列削峰 + 批量落库**，**车辆实时状态**（在线/离线 + 最新位置）驱动实时定位视图，**统一报警中心**（超速/温度/油量/疲劳/离线…规则引擎 + 去重），报警可喂给 Agent 归因。
...
- **IoT 终端接入网关**：**JT/T 808-2013** 位置汇报(0x0200)解析与构帧（纯函数 + 模拟器，含转义/校验和/坏帧拒收单测）、**MQTT 接入**（配 `MQTT_HOST` 即在网关内起订阅协程，不必另起进程；JT808 二进制帧与 JSON 文本按首字节自动分派），终端上报归一化后入现有削峰队列，复用批量落库 + 报警链路。
- **数据中台 · 指标中台**：统一指标注册表（运单/运力/订单/财务四主题域，口径集中）、指标查询 API（目录/多指标查询/趋势）、**按日物化快照**（网关内周期协程物化，含缺日回补）、经营看板前端；指标同源供 **AI Agent 直接调用**做经营分析（`analytics.query_metric` 工具）。
...
- **工程化**：Docker 多阶段构建（运行镜像只留静态二进制）、生产 Nginx + 网关编排、GitHub Actions CI（vet/build/test + 空库建库验证）、k6 压测脚手架。
...
- **去伪存真**：回单 / 证件 OCR 由随机数伪造改为诚实降级（待人工核验），杜绝“过期证件被洗白、假签收人误导财务核销”；监控页移除预置假报警，仅渲染真实告警；`dispatch` 端点改走状态机、修复多运单订单误判完单。
- **产品级前端**：全站配色 / 字体 / 排版重构，去除演示注释式文案。

## 本地一键运行（Docker）

> 宿主机无需装 Go/PG，只需 Docker。网关是唯一的应用进程。

```bash
# 1) 准备环境变量（可选，缺省值已能跑）
cp deploy/.env.example deploy/.env

# 2) 起全栈：Postgres + 网关（网关启动即跑内嵌迁移器：空库建基线，已有库只补增量）
docker compose -f deploy/docker-compose.yml up -d --build

# 3) 开首个超管
docker compose -f deploy/docker-compose.yml exec gateway /app/adminctl -u admin -p 'Admin12345!'

# 4) 验证
curl http://127.0.0.1:8000/healthz
```

不用 Docker 时（本机已有 PostgreSQL）：

```bash
cd backend-go
go run ./cmd/migrate                              # 建库/补增量
go run ./cmd/adminctl -u admin -p 'Admin12345!'   # 开首个超管
go run ./cmd/server                               # 起网关（:8000）
```

仓库里的 `scripts/dev/up.sh` 把上面三步连同 PostgreSQL 一起拉起，并把登录令牌写到
`/tmp/tok.txt`，本地调试直接用它。
...
生产预演（Nginx 网关 + Go 网关，端口 80）：

```bash
cp deploy/.env.example deploy/.env.prod   # 改强密钥/域名/口令
docker compose -f deploy/docker-compose.prod.yml --env-file deploy/.env.prod up -d --build
# http://127.0.0.1/
```

> 演示种子数据（`seed_demo` / `seed_org` / `seed_realistic`）原是 Django 管理命令，
> 随 Django 一并退役。需要造数时从 API 走正常业务链路，或从 git 历史里取回那几个脚本改写。
