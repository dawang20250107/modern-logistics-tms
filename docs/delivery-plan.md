> **架构已变更（本文档为历史交付方案，保留以说明设计取舍的来龙去脉）**
>
> 后端已从 Django 全量迁移到纯原生 Go：单进程网关（chi + pgx 裸 SQL）承载全部 API，
> schema 由内嵌迁移器接管，轨迹削峰与定时扫描改为进程内实现，因此 Redis 与 Celery
> 一并退出编排。逐端点的移植过程、契约对拍结论与刻意保留/修正的差异清单，
> 见 [backend-go/PORTING.md](../backend-go/PORTING.md)。
>
> 下文的技术选型段落描述的是迁移前的形态。
