...
不需要登录就能打到的业务端点一共 7 个。这是互联网直接能碰到的部分，
上线前应当逐个确认，扩容/加 WAF 时也要按这份清单配。

| 端点 | 把关方式 | 限流 |
|---|---|---|
| `POST /driver/login` | 手机号 + 口令 | `THROTTLE_DRIVER_LOGIN` 10/min |
| `GET /driver/tasks` | 司机令牌 | —— |
| `POST /driver/checkin` | 司机令牌 | —— |
| `POST /driver/credentials` | 司机令牌 | —— |
| `POST /driver/reminders/{id}/ack` | 司机令牌 | —— |
| `GET /track` | 订单号 + 手机号 | 见下 |
| `POST /public/orders` | 无 | `THROTTLE_PUBLIC_INTAKE` 5/min |

司机端五个端点实测均返回 401（不带令牌、带伪造令牌都拦），有用例钉着。

### `/track` 为什么要两道闸

这个端点的"密码"只是手机号后 4 位——SQL 允许 ≥4 位的后缀匹配，
是有意的可用性取舍，客户常常只记得后四位。而订单号是顺序可猜的。

**实测过：没有限流时，穷举 10000 种四位组合只要 60 秒。**
也就是任何人都能顺着单号把全系统的起止地、状态、时间线爬走——
对物流公司而言，那是把客户的发货规律直接交给同行。

两道闸各管一头：

- `THROTTLE_TRACK_ORDER`（默认 10/min，**按订单号**）——挡定向暴破。
  被反复试的是那个单号，攻击者换多少 IP 都绕不开。
  跟登录锁定按用户名而不是按 IP 是同一个道理。
- `THROTTLE_TRACK_FAIL_IP`（默认 20/min，**按来源 IP，且只对查不到的请求计数**）
  ——挡横向扫描（拿一批单号各试几次）。

第二道闸「只对失败计数」这一点很要紧，第一版没这么做，被用例抓了出来：
按 IP 无差别限流的话，国内大量用户共用出口 IP（企业 NAT、运营商 CGNAT），
30/min 摊到成百上千人身上立刻就满，**正常客户会查不了自己的单**。
只对失败计数就没有这个问题：客户知道自己的手机号，一次就查到，不占配额。

已知副作用：攻击者可以让**某一张**单的查询在一分钟内不可用。
这是可接受的取舍——另一头是让它可被暴破。

### 想更严可以调

- 把 `THROTTLE_TRACK_ORDER` 调低（如 `5/min`）
- 前面挂 WAF / CDN 限速，按真实流量调 `THROTTLE_TRACK_FAIL_IP`
- 观察 `/metrics` 上 `tms_http_requests_total{route="/api/v1/track",status="404"}`
  和 `status="429"` 的量，持续升高就是有人在扫
