...
## 4. OCR 引擎（**未接入**，交付时须明确告知）

回单与证件的 OCR 在代码里是**可插拔的空壳**：接口、状态机、落库字段、
前端展示都在，但**没有任何一家引擎被实际接上**。

三处调用点：

| 位置 | 触发 | 无引擎时的行为 |
|---|---|---|
| `resources/hooks.go: runReceiptOCR` | 上传回单后异步跑 | `ocr_status='manual'`，`fields` 为空 |
| `masterdata` 证件 OCR | 上传车辆/司机证件 | 同上 |
| `driver/handler.go: applyCredentialOCR` | 司机端自助上传证件 | 同上 |

`OCR_PROVIDER` 环境变量留了口子，但**填了也没用**——目前只会把
`note` 从「未配置 OCR 引擎」改成「OCR 引擎 X 尚未接入实现」，
状态一样是 `manual`。真正接入需要在 `recognizeReceipt` /
`applyCredentialOCR` 里实现具体厂商的调用。

### 为什么无引擎时不伪造字段

`runReceiptOCR` 的注释里写了，这里再强调一次：**未配置引擎时绝不填签收人
和签收时间**。回单是 e-POD 法律凭证，也是财务自动核销的依据；
伪造一个"识别出来的"签收人，等于凭空制造一份签收证明，
并可能触发对一笔实际未签收的运单核销。所以宁可留空、标 `manual`，
让人去录。

### 界面上的表现（2026-08 修正）

此前三处界面把「没有引擎」演成了「正在识别」：

- 运单详情的回单徽标：`ocr_status !== 'done'` 一律显示「提取中」，
  于是 `manual` 会**永远**显示「提取中」——一件不会完成的事
- 资源库上传证件后 toast 说「识别中」
- 司机端上传证件后 toast 说「识别建档中」

现已全部改为按真实状态显示（`manual` → 「待人工」，并说明本部署未接入 OCR）。
这与找回密码那处是同一类问题：**后端明确回报"没做"，前端却告诉用户"在做"**。
用户据此等待，而没有人会去做那件事。

### 交付说明必须写清楚

上线前与客户确认：证件与回单信息**需要人工录入**。
如果客户的验收标准里含「自动识别」，这是一个需要单独排期的集成项，
不是配一个环境变量就能开的开关。
